...
export function VoucherWalletModal({ isOpen, onClose, subtotal, onApply }: VoucherWalletModalProps) {
  const { data: vouchers, isLoading, error } = useGetWalletVouchers();

  return (
    <Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md w-[95vw] p-0 overflow-hidden sm:rounded-md bg-surface shadow-ui-card border-border">
        ...
        <div className="p-5 overflow-y-auto max-h-[60vh] bg-white space-y-4">
          ...
          {!isLoading && vouchers && vouchers.map((voucher) => {
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-ink uppercase">{voucher.code}</span>
                    <span className="text-xs font-semibold text-brand bg-brand/10 px-2 py-0.5 rounded-sm">
                      ...
                      {voucher.discountType === "fixed" && `Giảm ${voucher.discountValue.toLocaleString("vi-VN")}đ`}
                  ...
                  <p className="text-xs text-ink-muted line-clamp-2 mb-2">
                    ...
                    {voucher.maxDiscount && ` Giảm tối đa ${voucher.maxDiscount.toLocaleString("vi-VN")}đ.`}
                  </p>
                ...
                <div className="flex flex-col justify-center border-l border-dashed border-border pl-4">
                  <Button
                    ...
                    className={cn(
                      ...
                      isEligible ? "bg-brand text-white hover:bg-brand-hover shadow-ui-soft" : "bg-background text-ink-muted"
                    )}
                    onClick={() => {
                      if (isEligible) {
