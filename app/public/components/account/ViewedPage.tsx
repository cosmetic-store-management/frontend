...
import { useRecentlyViewed, useClearViewed, useRemoveViewed } from "@/public/hooks/useUser";
...
export function ViewedPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; productId: string | null; clearAll: boolean }>({
    open: false, productId: null, clearAll: false,
  });
    ...
    <div className="animate-slide-up bg-surface px-6 py-6 flex-1">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-ink-muted mt-0.5">Lịch sử duyệt sản phẩm của bạn</p>
        </div>
        {total > 0 && (
          <button
            onClick={() => setModal({ open: true, productId: null, clearAll: true })}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-ink mb-1">Chưa có sản phẩm nào trong lịch sử</p>
            <p className="text-xs text-ink-muted mb-4">Khám phá cửa hàng và xem các sản phẩm bạn yêu thích</p>
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products.map((product: any) => (
              <div key={product.id} className="relative group">
                ...
                <button
                  onClick={() => setModal({ open: true, productId: product.id, clearAll: false })}
                  className="absolute top-2 right-2 z-10 w-6 h-6 rounded-full bg-surface/90 border border-border shadow-sm text-ink-muted hover:text-danger hover:border-danger/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all"
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              ...
              <span className="text-sm text-ink-muted px-2">{page} / {totalPages}</span>
              <button
      ...
      <DeleteModal
        ...
        title={modal.clearAll ? "Xóa toàn bộ lịch sử?" : "Xóa sản phẩm khỏi lịch sử?"}
        description={
          modal.clearAll
        ...
        onClose={() => setModal({ open: false, productId: null, clearAll: false })}
        onConfirm={() => {
          if (modal.clearAll) {
            clearMutation.mutate(undefined, {
              onSuccess: () => { setModal({ open: false, productId: null, clearAll: false }); setPage(1); }
            });
          } else if (modal.productId) {
            removeMutation.mutate(modal.productId, {
              onSuccess: () => setModal({ open: false, productId: null, clearAll: false })
            });
          }
        }}
      />
