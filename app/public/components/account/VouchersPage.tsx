...
export function VouchersPage() {
  ...
  const expiredVouchers = allVouchers.filter((v: any) => v.status === "expired" || v.status === "exhausted");
  ...
  const activeVouchers = tab === "valid" ? validVouchers : tab === "used" ? usedVouchers : expiredVouchers;
          ...
          >
            ...
            {t.count > 0 && (
              <span className={`inline-flex items-center justify-center h-4 min-w-[16px] px-1 text-[10px] font-bold rounded-full ${
                tab === t.key ? "bg-brand text-white" : "bg-border/70 text-ink-muted"
              }`}>{t.count}</span>
            )}
          </button>
        ))}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <p className="text-sm text-ink-muted">
            {tab === "valid"   ? "Bạn chưa có mã khả dụng. Hãy lưu mã từ trang chủ!" :
             tab === "used"    ? "Bạn chưa sử dụng mã giảm giá nào." :
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {activeVouchers.map((voucher: any) => (
            <VoucherCard
              ...
              voucher={{ ...voucher, status: tab === "used" ? "expired" : undefined }}
            />
          ))}
        </div>
      )}
