...
export function ProductRecommendations({ productId }: ProductRecommendationsProps) {
  const { data: recommendations, isLoading } = useProductRecommendations(productId, 10);
  ...
  if (isLoading) {
    return (
      <div className="mt-12 py-8 border-t border-border">
        <h2 className="text-[18px] font-bold mb-4 text-ink uppercase">Đề xuất cho bạn</h2>
  ...
  return (
    <div className="bg-surface p-6 md:p-8 rounded-sm border border-border shadow-ui-soft w-full">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[20px] font-bold text-ink uppercase tracking-wider">Đề xuất cho bạn</h2>
      ...
      >
        {recommendations.map((product) => (
          <div key={product.id} className="w-[180px] min-w-[180px] sm:w-[200px] sm:min-w-[200px] md:w-[calc((100%-48px)/4)] md:min-w-[calc((100%-48px)/4)] lg:w-[calc((100%-64px)/5)] lg:min-w-[calc((100%-64px)/5)] snap-start shrink-0">
