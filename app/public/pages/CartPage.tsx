...
import { Trash2, Minus, Plus, ShoppingBag, ArrowRight, Ticket, ArrowLeft, Tag } from "lucide-react";
...
export function CartPage() {
  ...
  const handleApplyVoucher = async () => {
    ...
    try {
      const res = await validateVoucherMutation.mutateAsync({ code: voucherInput, subtotal });
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-[1200px] w-full mx-auto px-4 py-6 animate-page-enter">
        <div className="bg-white rounded-sm shadow-ui-card p-16 text-center">
          ...
          <p className="text-sm text-ink-muted mb-8">Hãy khám phá và thêm sản phẩm yêu thích vào giỏ hàng!</p>
          <Link
            to="/products"
    ...
    <div className="max-w-[1200px] w-full mx-auto px-4 py-6 animate-page-enter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] xl:grid-cols-[1fr_380px] gap-4 items-start">
        ...
        <div className="bg-white rounded-sm border border-border overflow-hidden">
          <div className="divide-y divide-border/50 overflow-y-auto max-h-[calc(100vh-240px)]">
            {items.map((item) => (
              <div key={`${item.productId}-${item.variantId}`} className="flex items-center gap-4 px-6 py-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between sm:hidden mt-2">
                    <div className="flex items-center border border-border rounded-sm h-7">
                      <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity - 1)} disabled={item.quantity <= 1} className="w-7 h-full flex items-center justify-center text-ink-muted hover:text-brand disabled:opacity-30"><Minus className="w-3 h-3" /></button>
                      <span className="w-7 text-center text-xs font-semibold text-ink">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity + 1)} disabled={item.stock > 0 && item.quantity >= item.stock} className="w-7 h-full flex items-center justify-center text-ink-muted hover:text-brand disabled:opacity-30 disabled:cursor-not-allowed"><Plus className="w-3 h-3" /></button>
                    </div>
                    <span className="font-bold text-sm text-ink">{(item.price * item.quantity).toLocaleString("vi-VN")}₫</span>
                  </div>
                </div>
                ...
                <div className="hidden sm:flex flex-col items-center shrink-0">
                  <div className="flex items-center border border-border rounded-sm h-8">
                    <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity - 1)} disabled={item.quantity <= 1} className="w-8 h-full flex items-center justify-center text-ink-muted hover:text-brand disabled:opacity-30 transition-colors">
                      <Minus className="w-3 h-3" />
                    </button>
                    <span className="w-8 text-center text-sm font-semibold text-ink select-none">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.productId, item.variantId, item.quantity + 1)} disabled={item.stock > 0 && item.quantity >= item.stock} className="w-8 h-full flex items-center justify-center text-ink-muted hover:text-brand disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
                      <Plus className="w-3 h-3" />
                    </button>
                  </div>
                  {item.stock > 0 && item.stock <= 5 && (
                    <span className="text-[10px] text-warning font-medium mt-0.5">Còn {item.stock}</span>
                  )}
                </div>
        ...
        <div className="bg-white rounded-sm border border-border overflow-hidden sticky top-24">
          ...
          <div className="px-6 py-5 border-b border-border">
            <h2 className="text-sm font-bold text-ink uppercase tracking-wider">Tổng đơn hàng</h2>
          </div>

          <div className="px-6 py-5 space-y-3">
            {/* Subtotal */}
            <div className="flex justify-between text-sm">
              <span className="text-ink-muted">Tạm tính ({items.reduce((s, i) => s + i.quantity, 0)} sản phẩm)</span>
              <span className="font-semibold text-ink">{subtotal.toLocaleString("vi-VN")}₫</span>
            </div>
            ...
            {cartStoreVoucherCode && (
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-brand">-{discountAmount.toLocaleString("vi-VN")}₫</span>
              </div>
            )}
            ...
            <div className="pt-3 border-t border-border/60 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-ink uppercase tracking-wide">Mã giảm giá</span>
                <button onClick={() => setIsWalletOpen(true)} className="text-xs font-semibold text-brand hover:underline flex items-center gap-1">
              ...
              {cartStoreVoucherCode ? (
                <div className="flex items-center justify-between bg-brand/5 border border-brand/20 px-3 py-2 rounded-sm">
                  ...
                  <button onClick={removeVoucher} className="text-xs text-danger hover:underline font-medium">Hủy</button>
                </div>
              ) : (
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    disabled={validateVoucherMutation.isPending || !voucherInput.trim()}
            ...
            <div className="pt-3 border-t border-border">
              <div className="flex items-baseline justify-between">
                <span className="font-bold text-sm text-ink">Tổng thanh toán</span>
                <div className="text-right">
                  <div className="text-xl font-bold text-brand">{total.toLocaleString("vi-VN")}₫</div>
                  <div className="text-[10px] text-ink-muted">Đã bao gồm VAT</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <VoucherWalletModal
        ...
        onApply={(code) => {
          ...
          validateVoucherMutation.mutate({ code, subtotal }, {
            onSuccess: (res) => {
              applyVoucher(res.result?.voucherCode, res.result?.discountAmount);
