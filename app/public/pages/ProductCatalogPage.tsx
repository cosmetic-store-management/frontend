    ...
    <div className="bg-white w-full flex-1">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <aside className="hidden lg:block w-64 flex-shrink-0">
          ...
          <div className="mb-6 border-b border-transparent lg:border-border/50 pb-4">
            ...
            <h2 className="text-2xl font-bold text-ink uppercase mt-1 tracking-tight">{sidebarTitle}</h2>
          </div>
          ...
          <div className="flex flex-col divide-y divide-border">
            ...
            {displaySubcategories.length > 0 && (
              <div className="py-6 first:pt-0">
                ...
                >
                  LOẠI SẢN PHẨM <ChevronDown className={`w-4 h-4 text-ink-muted transition-transform ${openFilters.categories ? "rotate-180" : ""}`} />
                </button>
                ...
                {displaySubcategories.map((cat) => (
                  <label key={cat.id || cat._id} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    />
                    <span className={`text-sm uppercase transition-colors flex items-center gap-1.5 ${selectedCategories.includes(cat.slug) ? "font-semibold text-ink" : "text-ink-muted group-hover:text-brand"}`}>
                      {cat.name}
                      <span className="text-xs text-ink-muted opacity-60 normal-case font-normal">({cat.productCount || 0})</span>
                    </span>
                  </label>
                ))}
              </div>
            ...
            <div className="py-6 first:pt-0">
              <button 
                ...
                className="w-full font-bold text-[14px] mb-4 uppercase flex justify-between items-center hover:text-brand transition-colors"
              >
                GIÁ <ChevronDown className={`w-4 h-4 text-ink-muted transition-transform ${openFilters.price ? "rotate-180" : ""}`} />
              </button>
              {openFilters.price && (
                <div>
            ...
            <button onClick={applyPriceFilter} className="w-full mt-4 bg-[#8A0000] hover:bg-brand-dark text-white font-bold py-2.5 rounded-sm transition-colors text-sm uppercase tracking-wide">
              Áp dụng
            </button>
                </div>
              )}
            ...
            {brands.length > 0 && (
              <div className="py-6 first:pt-0">
                <button 
                  ...
                  className="w-full font-bold text-[14px] mb-4 uppercase flex justify-between items-center hover:text-brand transition-colors"
                >
                  THƯƠNG HIỆU <ChevronDown className={`w-4 h-4 text-ink-muted transition-transform ${openFilters.brands ? "rotate-180" : ""}`} />
                </button>
                {openFilters.brands && (
                  <div>
                ...
                {[...brands]
                  ...
                  <label key={brand.id} className="flex items-center gap-3 cursor-pointer group">
                    <input 
                      ...
                      className="w-4 h-4 text-brand bg-surface border-border focus:ring-brand accent-brand rounded-sm cursor-pointer"
                    />
                    <span className={`text-sm transition-colors leading-tight ${selectedBrands.includes(brand.id) ? "font-semibold text-ink" : "text-ink-muted group-hover:text-brand"}`}>
                      {brand.name}
                    </span>
                  </label>
                ))}
              ...
              {selectedBrands.length > 0 && (
                <div className="mt-4 text-right">
                  <button onClick={resetBrands} className="text-brand hover:underline text-sm font-medium">
                    Đặt lại
                  </button>
                </div>
              )}
        ...
        <div className="flex-1">
            ...
            {[
              ...
              <button
                ...
                className={`flex-1 text-center text-sm md:text-base font-medium whitespace-nowrap transition-colors ${
                  sortBy === tab.id ? "text-brand" : "text-ink-muted hover:text-ink"
                }`}
              >
                {tab.label}
              </button>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 bg-surface">
              ...
              <h3 className="text-lg font-bold text-ink">Không tìm thấy sản phẩm</h3>
              <p className="text-ink-muted mt-2">Vui lòng thử lại với từ khóa hoặc bộ lọc khác.</p>
              <button 
            ...
            <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6 animate-stagger transition-opacity duration-200 ${isFetching ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
      ...
      {isMobileFilterOpen && (
        <div className="fixed inset-0 z-[60] bg-black/50 lg:hidden flex justify-end animate-fade-in">
          <div className="w-4/5 max-w-sm bg-surface h-full shadow-xl flex flex-col animate-slide-in-right">
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-8">
              {/* Category Filter */}
              <div>
                <h3 className="font-bold text-base mb-4 pb-2 border-b border-border">Danh mục</h3>
                <div className="space-y-4">
                  ...
                  {categories.map((cat) => (
                    <label key={cat.id || cat.slug} className="flex items-center gap-3">
                      <input 
                        type="radio" 
                        name="mobile_category" 
