...
import { Ticket, Tag, Truck, Check, Copy, Wallet, Gift, Clock, AlertCircle, LogIn } from "lucide-react";
import { usePublicVouchers, useCollectVoucher, useUncollectVoucher, useGetWalletVouchers } from "../hooks/useVoucher";
...
function VoucherCardFull({ voucher, isSaved, onCollect, onUncollect, isLoading }: {
  ...
  const isFull = voucher.usageLimit > 0 && voucher.usedCount >= voucher.usageLimit;
  ...
  return (
    <div className={`relative border rounded-sm flex overflow-hidden transition-all ${isFull ? "opacity-60" : ""} ${color}`}>
      {/* Notch left */}
      ...
      <div className="w-[30%] flex flex-col items-center justify-center py-5 border-r border-dashed border-current/30 shrink-0">
        ...
        <span className={`text-[11px] font-black tracking-wider uppercase px-2 py-0.5 rounded-sm ${badgeColor}`}>
          {badge}
        </span>
      </div>
      ...
      <div className="flex-1 p-4 flex flex-col gap-2">
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-ink-muted mt-0.5">
              Đơn tối thiểu {voucher.minOrderValue > 0 ? `${voucher.minOrderValue.toLocaleString("vi-VN")}₫` : "0₫"}
              {voucher.maxDiscount && voucher.maxDiscount > 0 && ` · Giảm tối đa ${voucher.maxDiscount.toLocaleString("vi-VN")}₫`}
            </p>
          </div>
        </div>
        ...
        {voucher.usageLimit > 0 && (
          <div>
            <div className="flex justify-between text-[10px] text-ink-muted mb-1">
              ...
              <span>{isFull ? "Đã hết" : `Còn ${voucher.usageLimit - voucher.usedCount} lượt`}</span>
            </div>
            <div className="h-1.5 bg-black/10 rounded-full overflow-hidden">
              <div className="h-full bg-current/50 rounded-full transition-all" style={{ width: `${usagePercent}%` }} />
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between mt-1 gap-2">
          {/* Expiry */}
          <div className={`flex items-center gap-1 text-[11px] ${isExpiringSoon ? "text-danger font-bold" : "text-ink-muted"}`}>
            <Clock className="w-3 h-3" />
            {isExpiringSoon ? `Còn ${daysLeft} ngày` : `HSD: ${new Date(voucher.endDate).toLocaleDateString("vi-VN")}`}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {!isFull && (
              <button
                onClick={() => isSaved ? onUncollect(voucher.code) : onCollect(voucher.code)}
                disabled={isLoading}
              ...
              >
                {isSaved ? <><Check className="w-3 h-3" /> Đã lưu</> : <><Gift className="w-3 h-3" /> Lưu mã</>}
              </button>
            )}
            {isFull && (
...
export function VouchersPage() {
  ...
  const { data: publicVouchers, isLoading: loadingPublic } = usePublicVouchers();
  const { data: walletVouchers, isLoading: loadingWallet } = useGetWalletVouchers();
  ...
  const displayedVouchers = tab === "all" ? (publicVouchers || []) : (walletVouchers || []);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-ink uppercase tracking-wide">Kho Mã Giảm Giá</h1>
        <p className="text-sm text-ink-muted mt-2">Lưu mã giảm giá và sử dụng khi thanh toán</p>
      </div>
      ...
      <div className="flex gap-1 bg-surface-soft border border-border rounded-sm p-1 mb-6 w-fit mx-auto">
        <button
          ...
          className={`px-6 py-2 text-sm font-semibold rounded-sm transition-all ${
            tab === "all" ? "bg-brand text-white shadow-sm" : "text-ink-muted hover:text-ink"
          }`}
        >
          <span className="flex items-center gap-1.5"><Gift className="w-4 h-4" /> Tất cả mã</span>
        </button>
        <button
          onClick={() => setTab("wallet")}
          className={`px-6 py-2 text-sm font-semibold rounded-sm transition-all ${
            tab === "wallet" ? "bg-brand text-white shadow-sm" : "text-ink-muted hover:text-ink"
          }`}
        >
      ...
      {tab === "wallet" && !user && (
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <Link to="/auth/login" className="px-6 py-2.5 bg-brand text-white font-bold rounded-sm hover:bg-brand-dark transition-colors text-sm">
            Đăng nhập ngay
          </Link>
        </div>
      )}
      ...
      {isLoading && (
        <div className="grid gap-3 sm:grid-cols-2">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-28 bg-surface-soft border border-border rounded-sm animate-pulse" />
          ))}
        </div>
      )}

      {/* Empty state */}
      {!isLoading && (!displayedVouchers || displayedVouchers.length === 0) && (tab === "all" || user) && (
        <div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <p className="text-ink-muted text-sm">
            {tab === "wallet" ? "Bạn chưa lưu mã nào. Hãy khám phá và lưu mã ở tab \"Tất cả mã\"!" : "Hiện chưa có mã giảm giá nào."}
          </p>
          {tab === "wallet" && (
            <button
              onClick={() => setTab("all")}
