...
export function getProducts(query: ProductQuery = {}): Promise<ProductListResult> {
  return apiClient.get<ProductListResult>("/products", query as Record<string, string | number | boolean>);
}

export function getProductBySlug(slug: string): Promise<Product> {
  return apiClient.get<{ product: Product }>(`/products/${slug}`)
    .then((data) => data.product);
}

export function getRecommendedProducts(id: string, limit: number = 10): Promise<Product[]> {
  return apiClient.get<{ products: Product[] }>(`/products/${id}/recommendations?limit=${limit}`)
