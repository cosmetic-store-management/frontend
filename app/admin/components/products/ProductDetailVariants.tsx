      ...
      <div className="overflow-x-auto border border-border rounded-sm">
        <table className="w-full text-xs table-auto">
          <thead className="bg-surface-soft text-ink-muted">
            <tr>
              <th className="px-2 py-1.5 font-medium text-center">{"Name"}</th>
              <th className="px-2 py-1.5 font-medium text-center">Barcode</th>
              <th className="px-2 py-1.5 font-medium text-center">{"Price"}</th>
              <th className="px-2 py-1.5 font-medium text-center">Stock</th>
            ...
            {variants.map((v, i) => {
              ...
              return (
                <tr key={i} className="hover:bg-surface-soft/50">
                  <td className="px-2 py-1.5 font-medium text-ink text-center">{v.name}</td>
                    ...
                    {displayBarcode ? (
                      <div className="flex flex-col items-center gap-1">
                        <img
                          ...
                          className="h-5 max-w-[80px] object-contain mx-auto"
                          loading="lazy"
                        />
                        <span className="text-[8px] font-mono text-ink-muted leading-none block text-center">
                          {displayBarcode}
                        </span>
                      </div>
                    ) : (
                ...
                <td className="px-2 py-1.5 text-center">
                  <span
                    className={`inline-flex px-1.5 py-0.5 rounded-[4px] font-semibold text-[9px] uppercase tracking-wider ${
                      v.isActive !== false
