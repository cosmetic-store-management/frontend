...
import { createStaffSchema, type CreateStaffFormData } from "../../schemas/user.schema";
...
export function StaffFormModal({ open, onOpenChange, onSubmit, isLoading }: StaffFormModalProps) {
  const { isOwner } = useAuth();

  const { control, handleSubmit, formState: { errors }, reset, watch, setValue } = useForm<CreateStaffFormData>({
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    resolver: zodResolver(createStaffSchema) as any,
    defaultValues: { name: "", email: "", phone: "", role: "staff", permissions: DEFAULT_STAFF_PERMISSIONS },
  });

  const selectedRole = watch("role");

  // Reset form when modal opens
  if (open && !isLoading && Object.keys(errors).length === 0 && !control._defaultValues.name) {
    // Small trick to avoid resetting on every render
  }

  const handleOpenChange = (isOpen: boolean) => {
    if (!isOpen) {
      reset({ name: "", email: "", phone: "", role: "staff", permissions: DEFAULT_STAFF_PERMISSIONS });
    }
    onOpenChange(isOpen);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl animate-scale-in text-left max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-base font-bold text-ink">Thêm tài khoản nhân viên mới</DialogTitle>
          <DialogDescription className="text-xs text-ink-muted mt-1">
            Lưu thông tin cá nhân nhân viên. Sau khi lưu, hệ thống sẽ cấp một tài khoản với mật khẩu mặc định là <code className="bg-surface-muted px-1 py-0.5 rounded-sm text-brand font-bold text-[10px]">GlowUp@123456</code>.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Controller
              ...
              name="name"
              render={({ field }) => (
                <Input {...field} id="sName" placeholder="Ví dụ: Nguyễn Văn A" />
              )}
            />
          ...
          <div className="space-y-1.5">
            ...
            <Controller
              ...
              name="email"
              render={({ field }) => (
                <Input {...field} id="sEmail" type="email" placeholder="Ví dụ: staff.a@glowup.vn" />
              )}
            />
            ...
            <Controller
              ...
              render={({ field }) => (
                <div className="flex flex-col space-y-2">
                  <label className={`flex items-start space-x-3 p-3 rounded-sm border cursor-pointer transition-colors ${field.value === 'staff' ? 'bg-surface-soft border-brand' : 'bg-surface border-border hover:bg-surface-hover'}`}>
                  ...
                  {isOwner && (
                    <label className={`flex items-start space-x-3 p-3 rounded-sm border cursor-pointer transition-colors ${field.value === 'manager' ? 'bg-surface-soft border-brand' : 'bg-surface border-border hover:bg-surface-hover'}`}>
                      <div className="flex items-center h-5">
                        <input
                          type="radio"
                          value="manager"
          ...
          <div className="pt-4 border-t border-border mt-4">
              ...
              <Select
                ...
                value={
                  selectedRole === "manager" 
                    ? (control._formValues.permissions === DEFAULT_MANAGER_PERMISSIONS ? "store_manager" : "custom")
                    : (control._formValues.permissions === DEFAULT_STAFF_PERMISSIONS ? "sales" : "custom")
                }
              >
            ...
            <div className="overflow-x-auto border border-border rounded-sm mt-4">
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody>
                  {MODULES.filter(m => m.allowedRoles.includes(selectedRole)).map((module) => (
                                ...
                                render={({ field }) => (
                                  <Checkbox
                                    ...
                                    onCheckedChange={(checked) => {
                                      ...
                                      field.onChange(checked ? [...val, permId] : val.filter((v: string) => v !== permId));
                                    }}
          ...
          <DialogFooter className="mt-4">
            <Button type="button" variant="outline" onClick={() => handleOpenChange(false)}>Huỷ</Button>
