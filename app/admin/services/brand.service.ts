...
export function getAdminBrands(
  ...
  return apiClient.get<AdminBrandListResult>(
    "/brands/admin/list",
...
export function getBrandDetail(id: string): Promise<Brand> {
  return apiClient
    .get<{ brand: Brand }>(`/brands/admin/${id}`)
    .then((res) => res.brand);
}

export function createBrand(
  data: Omit<Brand, "id" | "slug" | "productCount">,
): Promise<Brand> {
  return apiClient
    .post<{ brand: Brand }>("/brands/admin", data)
    .then((res) => res.brand);
}

export function updateBrand(
  id: string,
  data: Partial<Omit<Brand, "id" | "slug">>,
): Promise<Brand> {
  return apiClient
    .patch<{ brand: Brand }>(`/brands/admin/${id}`, data)
    .then((res) => res.brand);
}
...
export function updateBrandStatus(
  ...
  return apiClient
    .patch<{ brand: Brand }>(`/brands/admin/${id}/status`, { isActive })
    .then((res) => res.brand);
}
