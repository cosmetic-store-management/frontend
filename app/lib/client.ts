...
const BASE_URL = (import.meta.env.VITE_API_URL || "http://localhost:8000/api").trim();
...
function buildHeadersWithToken(token: string | null, isJson = true): HeadersInit {
  const headers: Record<string, string> = { Accept: "application/json" };
  if (isJson) headers["Content-Type"] = "application/json";
  if (token)  headers["Authorization"] = `Bearer ${token}`;
  return headers;
  ...
  if (res.status === 401 && !isAuthRoute) {
    ...
    if (newToken) {
      ...
      if (retryRes.ok) {
        ...
        if (json && json.success !== undefined && json.message && json.data && typeof json.data === "object" && !Array.isArray(json.data)) {
    ...
    if (hadToken && typeof window !== "undefined") {
      ...
      if (currentPath.startsWith("/admin")) {
        if (currentPath !== "/admin/login") window.location.href = "/admin/login";
      } else {
...
export const apiClient = {
  get<T>(
    ...
    const doRequest = () => fetch(url, { headers: buildHeaders(false) });
