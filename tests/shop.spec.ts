...
test.describe("Shop — Trang chủ", () => {
  test("[Happy] Trang chủ load thành công và có nội dung", async ({ page }) => {
    ...
    const content = page.locator("main, [data-testid='homepage'], .home-page, h1, h2").first();
...
test.describe("Shop — Danh sách sản phẩm", () => {
  test("[Happy] Trang /products load và hiển thị danh sách", async ({ page }) => {
    await page.goto("/products");

    await expect(page).toHaveURL(/products/);
    // Chờ ít nhất 1 sản phẩm hoặc thông báo trống
    const productList = page.locator("[data-testid='product-card'], .product-card, article, .grid > *").first();
  ...
  test("[Happy] Tìm kiếm sản phẩm hoạt động", async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const hasSearch = page.url().includes("search") || page.url().includes("q=");
      const listUpdated = await page.locator("[data-testid='product-card'], article").count();
      expect(hasSearch || listUpdated >= 0).toBeTruthy();
    }
  });

  test("[Error] Tìm kiếm không tìm thấy hiển thị thông báo trống", async ({ page }) => {
    await page.goto("/products?search=sanphamkhongtontai99999");
    await page.waitForTimeout(2_000);

    // Phải có empty state message
    const emptyState = page.locator("[data-testid='empty'], .empty-state, :text('không tìm thấy'), :text('Không có'), :text('0 sản phẩm')").first();
    const hasNoProducts = (await page.locator("[data-testid='product-card'], article").count()) === 0;
    expect(hasNoProducts || await emptyState.isVisible().catch(() => false)).toBeTruthy();
  });

  test("[Happy] Phân trang hoạt động (nếu có nhiều sản phẩm)", async ({ page }) => {
    await page.goto("/products");
    await page.waitForTimeout(1_500);

    const pagination = page.locator("[data-testid='pagination'], nav[aria-label*='page'], .pagination").first();
    if (await pagination.isVisible()) {
      const nextBtn = page.locator("button[aria-label*='next'], button:has-text('›'), a:has-text('Tiếp')").first();
...
test.describe("Shop — Chi tiết sản phẩm", () => {
  test("[Happy] Trang chi tiết sản phẩm hiển thị đủ thông tin", async ({ page }) => {
    await page.goto("/products");
    await page.waitForTimeout(1_500);
    ...
    if (await firstProduct.isVisible()) {
      ...
      const price = page.locator("[data-testid='price'], .price, :text('₫'), :text('đ')").first();
  ...
  test("[Happy] Thêm vào giỏ hàng thành công", async ({ page }) => {
    ...
    if (await firstProduct.isVisible()) {
      ...
      if (await addToCartBtn.isVisible()) {
        ...
        const feedback = page.locator("[role='alert'], [data-testid='cart-count'], .cart-badge, .toast").first();
  ...
  test("[Error] URL sản phẩm không tồn tại trả về 404 hoặc redirect", async ({ page }) => {
    const response = await page.goto("/products/san-pham-khong-ton-tai-abc-xyz-9999");
    // Có thể trả về 404 page hoặc redirect
    const is404 = response?.status() === 404;
    const hasNotFoundContent = await page.locator(
...
test.describe("Shop — Giỏ hàng", () => {
  ...
  test("[Error] Checkout khi giỏ hàng trống thông báo lỗi hoặc disable button", async ({ page }) => {
    await page.goto("/cart");
    await page.waitForTimeout(1_000);
    ...
    if (await checkoutBtn.isVisible()) {
      ...
      if (!isDisabled) {
        ...
        const showsError  = await page.locator("[role='alert']").first().isVisible().catch(() => false);
